...
const listContainer = list({
	...
	height: '100%',
	content:
		'Whoopsy! Something went wrong. Try quitting and restarting the program.',
	border: 'line'
});
...
async function rightHandler() {
	...
	if (selected.isDirectory() || selected.isSymbolicLink()) {
		...
		boxContainer.setContent(
			(fileContent.length * 2 + 2) > 128 * 1024 ? fileContent : await highlight(fileContent)
		);
		listContainer.hide();
		boxContainer.show();
		win.render();
