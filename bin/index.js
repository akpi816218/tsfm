...
const { blueBright, bold, cyanBright, greenBright, magentaBright, redBright, underline, unstyle, yellowBright } = ansiColors;
...
async function renderDir() {
    ...
    State.currentFileNames = State.currentFiles.map((file) => {
        if (file.isDirectory())
            return blueBright(`${file.name}/`).c;
        if (file.isSymbolicLink())
            return cyanBright(`${file.name}@`).c;
...
async function rightHandler() {
    ...
    if (selected.isDirectory() || selected.isSymbolicLink()) {
        ...
        boxContainer.setContent(((fileContent.length * 2 + 2) > 128 * 1024) || NOCOLOR ? fileContent : await highlight(fileContent).c);
        listContainer.hide();
        boxContainer.show();
        win.render();
    }
...
async function plopHandler() {
    ...
    modalContainer.setContent(yellowBright(`Sucessfully plopped ${filedata.originalPath}!\nThe file can be plunged using \n'feces plunge ${filedata.trashedPath
        .split('/')
        .at(-1)}' (assuming you have installed 'feces-cli').`).c);
    modalContainer.show();
    win.render();
